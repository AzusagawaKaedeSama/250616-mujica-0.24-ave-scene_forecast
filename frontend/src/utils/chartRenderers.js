...
export function renderIntervalForecastChart(data, containerId, forecastType) {
  ...
  if (data.predictions.some(p => p.upper_bound !== undefined && p.lower_bound !== undefined)) {
    console.log('添加置信区间阴影');
    
    // 准备自定义渲染数据
    const confidenceData = [];
    for (let i = 0; i < data.predictions.length; i++) {
      const upperValue = Number(data.predictions[i].upper_bound);
      const lowerValue = Number(data.predictions[i].lower_bound);
      
      // 每个数据点包含[x索引, 下边界值, 上边界值]
      confidenceData.push([i, lowerValue, upperValue]);
    }
    ...
    seriesData.push({
      ...
      renderItem: function(params, api) {
        ...
        const xCoord = api.coord([xValue, lowerValue]);
        const yLowerCoord = xCoord[1];
        const yUpperCoord = api.coord([xValue, upperValue])[1];
        
        const barWidth = api.size([1, 0])[0] * 0.8;
        
        return {
          type: 'rect',
          shape: {
            x: xCoord[0] - barWidth / 2,
            y: yUpperCoord,
            width: barWidth,
            height: yLowerCoord - yUpperCoord
          },
          style: api.style({
  ...
  if (data.predictions.some(p => p.predicted !== undefined && p.predicted !== null && !isNaN(p.predicted))) {
    ...
    seriesData.push({
      name: '点预测',
      type: 'line',
      data: data.predictions.map(p => Number(p.predicted)),
      smooth: true,
      symbol: 'none',
      lineStyle: {
        width: 3,
  ...
  if (data.predictions.some(p => p.upper_bound !== undefined && p.lower_bound !== undefined)) {
    ...
    seriesData.push({
      name: '上边界',
      type: 'line',
      data: data.predictions.map(p => Number(p.upper_bound)),
      smooth: true,
      symbol: 'none',
      lineStyle: {
        width: 2,
    ...
    seriesData.push({
      name: '下边界',
      type: 'line',
      data: data.predictions.map(p => Number(p.lower_bound)),
      smooth: true,
      symbol: 'none',
      lineStyle: {
        width: 2,
  ...
  if (data.predictions.some(p => p.actual !== undefined && p.actual !== null && !isNaN(p.actual))) {
    ...
    seriesData.push({
      name: '实际值',
      type: 'line',
      data: data.predictions.map(p => p.actual !== undefined && p.actual !== null ? Number(p.actual) : null),
      smooth: true,
      symbol: 'circle',
      symbolSize: 4,
      lineStyle: {
  ...
  const option = {
    ...
    legend: {
      data: seriesData.filter(s => s.name !== '置信区间' || s.type !== 'custom').map(s => s.name),
      bottom: 10,
      textStyle: {
        color: '#ffffff'
      },
  ...
  const chart = echarts.init(container); // 使用 'dark' 主题
