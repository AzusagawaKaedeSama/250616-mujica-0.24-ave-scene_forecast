...
class DIContainer:
    ...
    def __init__(self, use_real_implementations: bool = True):
        ...
        if use_real_implementations:
            print("Initializing DI Container with REAL implementations")
            self._init_real_implementations()
        else:
            print("Initializing DI Container with IN-MEMORY implementations")
...
def create_app(container: DIContainer) -> Flask:
    ...
    def get_training_status(task_id):
        ...
        """
        try:
            # 在真实环境中，这应该查询真实的训练任务状态
            status = container.training_service.get_training_status(task_id)
            ...
            return jsonify({
                ...
                "data": {
                    ...
                    "message": status.get("message", "Training completed successfully")
                }
            })
